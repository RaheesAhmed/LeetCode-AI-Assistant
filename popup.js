document.addEventListener('DOMContentLoaded', function() {
  ...
  // Load saved settings
  chrome.storage.sync.get(['aiProvider', 'apiKey', 'assistanceType'], function(result) {
    if (result.aiProvider) aiProviderSelect.value = result.aiProvider;
    if (result.apiKey) apiKeyInput.value = result.apiKey;
    if (result.assistanceType) assistanceTypeSelect.value = result.assistanceType;
  });
  ...
  chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
    ...
    if (currentTab.url.includes('leetcode.com/problems/')) {
      ...
      chrome.tabs.sendMessage(currentTab.id, {action: 'getProblemDetails'}, function(response) {
        if (response && response.problemDetails) {
          ...
          // Get current code
          chrome.tabs.sendMessage(currentTab.id, {action: 'getCurrentCode'}, function(codeResponse) {
          ...
          // Get test cases
          chrome.tabs.sendMessage(currentTab.id, {action: 'getTestCases'}, function(testResponse) {
          ...
          chrome.tabs.sendMessage(currentTab.id, {action: 'getSubmissionResults'}, function(resultResponse) {
          ...
          problemDetailsDiv.innerHTML = '<p>Error retrieving problem details. Please refresh the page.</p>';
        }
      });
    } else {
      problemDetailsDiv.innerHTML = '<p>Not on a LeetCode problem page. Please navigate to a problem.</p>';
      disableAllButtons();
    }
  });
  ...
  getHelpButton.addEventListener('click', function() {
    ...
    chrome.storage.sync.get(['aiProvider', 'apiKey', 'assistanceType'], function(settings) {
      if (!settings.apiKey) {
          ...
          if (response && response.problemDetails) {
            ...
            chrome.runtime.sendMessage({
              ...
              if (aiResponse && aiResponse.content) {
                ...
                if (settings.assistanceType === 'solution' || settings.assistanceType === 'optimize') {
                  insertCodeButton.classList.remove('hidden');
                }
              } else {
                showNotification('Error getting AI response. Please check your API key and try again.', 'error');
              }
            });
          } else {
            hideLoading();
            showNotification('Error retrieving problem details. Please refresh the page.', 'error');
          }
        });
      });
    });
  ...
  optimizeCodeButton.addEventListener('click', function() {
    ...
    chrome.storage.sync.get(['aiProvider', 'apiKey'], function(settings) {
      ...
      chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
        chrome.tabs.sendMessage(tabs[0].id, {action: 'getCurrentCode'}, function(response) {
          if (response && response.code) {
            ...
            chrome.runtime.sendMessage({
              ...
              if (aiResponse && aiResponse.content) {
                ...
                showNotification('Error optimizing code. Please check your API key and try again.', 'error');
              }
            });
          } else {
            hideLoading();
            showNotification('Error retrieving your code. Please make sure you have code in the editor.', 'error');
          }
        });
      });
    });
          ...
          chrome.tabs.sendMessage(tabs[0].id, {action: 'getCurrentCode'}, function(response) {
            if (response && response.code) {
              ...
              if (currentTestCases.length === 0) {
                chrome.tabs.sendMessage(tabs[0].id, {action: 'getTestCases'}, function(testResponse) {
                  if (testResponse && testResponse.testCases) {
                    currentTestCases = testResponse.testCases;
                  }
                  
                  // Send to background script to make API call
                  sendDebugRequest(settings, currentCode, currentTestCases, currentProblemDetails);
                });
              } else {
                // Send to background script to make API call
                sendDebugRequest(settings, currentCode, currentTestCases, currentProblemDetails);
  ...
  if (explainErrorButton) {
    explainErrorButton.addEventListener('click', function() {
      ...
      chrome.storage.sync.get(['aiProvider', 'apiKey'], function(settings) {
        ...
        chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
          chrome.tabs.sendMessage(tabs[0].id, {action: 'getCurrentCode'}, function(codeResponse) {
            if (codeResponse && codeResponse.code) {
              currentCode = codeResponse.code;
              
              // Get submission results if we don't have them yet
              if (!currentSubmissionResults) {
                chrome.tabs.sendMessage(tabs[0].id, {action: 'getSubmissionResults'}, function(resultResponse) {
                  if (resultResponse && resultResponse.results) {
                    currentSubmissionResults = resultResponse.results;
                    sendExplainErrorRequest(settings, currentCode, currentSubmissionResults, currentProblemDetails);
                  } else {
                    hideLoading();
                    showNotification('No error found. Please run your code first to get an error message.', 'warning');
                  }
                });
              } else {
                sendExplainErrorRequest(settings, currentCode, currentSubmissionResults, currentProblemDetails);
  ...
  insertCodeButton.addEventListener('click', function() {
    const codeMatch = aiResponseDiv.innerText.match(/```(?:[\w]*)\n([\s\S]*?)```/);
      ...
      chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
        chrome.tabs.sendMessage(tabs[0].id, {
          ...
          if (response && response.success) {
            ...
            showNotification('Failed to insert code. Please try again.', 'error');
          }
        });
      });
    } else {
      showNotification('No code found in the AI response to insert.', 'warning');
    }
  });
  ...
  function displayProblemDetails(details) {
    problemDetailsDiv.innerHTML = `
      ...
      <p><strong>Description:</strong> ${details.description.substring(0, 100)}...</p>
      ${details.topics && details.topics.length > 0 ? `<p><strong>Topics:</strong> ${details.topics.join(', ')}</p>` : ''}
    `;
  }
  ...
  function sendDebugRequest(settings, code, testCases, problemDetails) {
    chrome.runtime.sendMessage({
      ...
      if (aiResponse && aiResponse.content) {
        ...
        showNotification('Error debugging code. Please check your API key and try again.', 'error');
      }
    });
  }

  function sendExplainErrorRequest(settings, code, submissionResults, problemDetails) {
    const errorMessage = submissionResults.errorDetails || submissionResults.message || 'Unknown error';
    ...
    chrome.runtime.sendMessage({
      ...
      if (aiResponse && aiResponse.content) {
        ...
        showNotification('Error explaining the error. Please check your API key and try again.', 'error');
      }
    });
  }
